        ...
        function (error, response, body) {
            ...
            try {
                var jsonBody = JSON.parse(body),
                    status = jsonBody.response.request.status;
                //if (parseInt(status.code, 10) >= 400) {
                //}
...
Api.prototype.get = function (requestUrl, token, qs, callback) {
    ...
    this.request.call(this, request.get, requestUrl, params, callback);
};
 ...
 * @param  {Function} callback   the callback function
 */
Api.prototype.post = function (requestUrl, token, body, callback) {
    var params = {
        token: token,
        body: {request: body}
    };
...
Api.prototype.retrieveData = function (error, jsonBody, tag, callback) {
    ...
    try {
        if (jsonBody.response.result) {
            if (tag !== undefined !== undefined && jsonBody.response.result.data[tag] !== undefined) {
