...
function initiate(sharedMessageToken, pingbackUri, callback) {
    ...
    if (pingbackUri !== undefined) {
        body.analytics.sharing.pingback_uri = pingbackUri;
    }

    this.api.put('sharing/analytics/snapshots', undefined, body, function (error, result) {
        this.api.retrieveData(error, result, undefined, callback);
    }.bind(this));
}
...
function getAll(page, callback) {
    this.api.get('sharing/analytics/snapshots', undefined, {page:page}, function (error, result) {
